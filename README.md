...
需要 Docker 和 systemd。使用 root 用户执行一键安装命令：
...
`aethercloud-v6.service`。以后升级时重新执行同一条命令即可，现有配置和 SOCKS5
密码会保留，并会拉取最新镜像后重启服务。安装完成后，终端会输出一份包含实际
用户名和密码的 sing-box 出站 JSON，可直接复制到 `custom/04_outbounds.json`。
