...
- 发布脚本版本更新为 `v1.3.23`。
- `SERVER_IP` 明确定义为导出给客户端的服务器地址，除公网 IPv4/IPv6 外支持 DDNS 域名；安装、Docker 和配置菜单共用严格校验。
- 端口占用检测改为解析 `ss` 的本地监听端口并做完整数值比较，避免端口 `1111` 被监听中的 `11111` 误判为占用。
- 上游 `v1.3.20` 中其余独立协议端口、WARP/OpenAI 检测和 nginx 生命周期改动不重复迁移：下游已有更完整实现或已明确删除对应行为。
- `v1.3.22` 新增显式启用的公网 SOCKS5 入站，监听 IPv4/IPv6，支持 TCP/UDP，并强制用户名和密码认证，不提供匿名模式。
...
- 发布脚本版本更新为 `v1.3.19`。
- 借鉴上游重写后的 `56997fd` 升级兼容思路，把项目管理的日志、缓存、DNS 和 HTTP 客户端基础配置抽成可复用生成器。
- VPS 使用新内核检查失败时，会在临时目录重建托管基础配置并从 `custom/` 重新生成路由候选；日志等级、DNS 策略、协议入站和 `custom/` 源文件保持不变，候选通过新内核检查且用户确认后才应用。
- VPS 内核与完整运行配置改为事务升级；停止、文件切换或新服务启动失败时，同时恢复旧内核和旧配置，并再次确认旧服务可运行。
- 不采用上游直接覆盖 `conf/0*`、默认同意重置、恢复旧内核却丢弃配置备份的实现，也不重新引入 WARP 或 NTP。
- 将版本查询失败时的兜底版本从不支持当前 `http_clients` 配置的 `1.13.0-rc.4` 更新为已通过真实配置检查的 `1.14.0-alpha.50`。
- 新增升级兼容与回滚测试，覆盖托管配置重建、`custom/` 不变、用户偏好保留、成功切换和新服务失败后的完整恢复。
