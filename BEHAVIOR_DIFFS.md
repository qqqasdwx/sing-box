...
- 本次 review 不移植上游 `bb5deb9` 的 VPS 运行时订阅开关。本仓库已通过 `SUBSCRIBE` 覆盖 VPS 配置更新和 Docker 环境变量；上游实现重新下载独立仓库中的二维码可执行文件，并把订阅状态与 nginx 配置内容耦合。如以后增加菜单开关，应复用现有配置状态和服务生命周期，不恢复二维码依赖。
- 本次 review 不移植上游 `eb5c5a3` 的消息数组编号整理；本仓库已将中英文消息和运行逻辑拆分为模块，编号与上游没有可直接同步的关系。
- 本次 review 不移植上游 `0903648` 的 clash API 流量统计。该数据只覆盖 sing-box 当前进程生命周期，重启后归零；上游还会在脚本启动时自动改写旧 `04_experimental.json` 并立即重载服务，不符合本仓库的检查发布流程。其 GitHub 代理 HEAD 探测也不适用于本仓库显式配置 `GH_PROXY` 的策略。
- 本次 review 不移植上游 `8d893d6` 的零协议运行模式；本仓库继续要求至少保留一个入站，避免服务显示正常却没有可用节点。只移植其中适用于现有行为的空通配符处理、无用代码清理，以及删除最后一个 WS 时保留 Token/JSON 固定 Argo Tunnel 的逻辑。
...
| VPS 运行时订阅开关 | `sb -d` 根据 nginx 配置内容开启或关闭订阅，并按需下载二维码程序、增删 nginx 配置和修改服务文件。 | 通过 `SUBSCRIBE` 配置项执行完整配置更新；暂不提供菜单快捷开关。 | 订阅状态需要和配置状态、Argo、WS、nginx、服务文件及防火墙保持一致，不应只根据 nginx 文本推断或恢复第三方二维码依赖。 |
| 流量统计 | 默认启用本地 clash API，在 `sb -n` 和主菜单显示当前进程累计流量；旧配置会在脚本启动时自动改写并重载。 | 不启用 clash API，也不展示进程周期流量。 | 进程重启即归零的数据不能作为长期流量统计；为展示信息而自动修改运行配置不符合显式检查发布原则。 |
